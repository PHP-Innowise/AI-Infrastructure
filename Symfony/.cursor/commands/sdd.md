---
name: sdd
description: "Drive spec-driven development from the main conversation - specify, design, break into tasks, then execute task by task - by spawning this accelerator's own agents stage by stage, leaving a durable artifact in `specs/` and `tasks/` at every phase so the work can be reviewed and resumed in a later session."
---
...
Read `.cursor/skills/sdd/SKILL.md` first: it defines the artifact paths, the
