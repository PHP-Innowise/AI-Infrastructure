...
Shared Symfony workflow behavior is authored canonically in `.claude/skills` and mirrored into `.cursor/skills` and `.agents/skills`. Each mirror must preserve the workflow contract while adapting paths, frontmatter, commands, agents, hooks, and tool-integrated behavior to the target platform.
