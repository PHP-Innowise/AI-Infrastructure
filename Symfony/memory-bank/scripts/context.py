...
def connect(database: Path) -> sqlite3.Connection:
    ...
    try:
        connection.executescript(
            ...
            CREATE TRIGGER IF NOT EXISTS episodes_insert AFTER INSERT ON episodes BEGIN
                INSERT INTO episodes_fts(rowid, summary, outcome)
                VALUES (new.id, new.summary, new.outcome);
            END;

            CREATE TRIGGER IF NOT EXISTS episodes_delete AFTER DELETE ON episodes BEGIN
                INSERT INTO episodes_fts(episodes_fts, rowid, summary, outcome)
                VALUES ('delete', old.id, old.summary, old.outcome);
            END;

            CREATE TRIGGER IF NOT EXISTS episodes_update AFTER UPDATE ON episodes BEGIN
                INSERT INTO episodes_fts(episodes_fts, rowid, summary, outcome)
                VALUES ('delete', old.id, old.summary, old.outcome);
                INSERT INTO episodes_fts(rowid, summary, outcome)
                VALUES (new.id, new.summary, new.outcome);
...
def index_repository(connection: sqlite3.Connection, repository: Path) -> dict[str, int]:
    ...
    with connection:
        for path, kind, title, content in documents:
            connection.execute(
                """
                INSERT INTO documents(path, kind, title, content, content_hash, updated_at)
                VALUES (?, ?, ?, ?, ?, ?)
...
def search_documents(
    ...
    rows = connection.execute(
        ...
        ORDER BY bm25(documents_fts), documents.path
        LIMIT ?
        """,
...
def search_episodes(
    ...
    rows = connection.execute(
        ...
        ORDER BY bm25(episodes_fts), episodes.id DESC
        LIMIT ?
        """,
        (fts_query(query), limit),
    ).fetchall()
