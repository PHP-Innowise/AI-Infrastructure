---
name: memory-bank
description: Manage durable WordPress project memory. Use to retrieve or capture verified reusable knowledge, audit or supersede stale chunks, or apply a governed automatic or independently reviewed Project Brain promotion. Do not use for active task state, handoffs, raw transcripts, or unverified facts.
phase: utility
flow-next: null
flow-alternatives: [project-brain, documentation-generator, reflect]
---

# WordPress Memory Bank
...
1. Confirm the candidate is durable, reusable, project-specific, and safe to commit. Reject transient status, speculative reasoning, generic WordPress advice, and duplicated spec content.
...
## WordPress Memory Categories

Use a concise `type` such as:

- `architecture`: established plugin/theme/block boundaries, lifecycle entry points, services, data ownership, or package ownership;
...
- `operations`: verified deployment, cron/scheduled-action, migration, cache, monitoring, or recovery lesson.
