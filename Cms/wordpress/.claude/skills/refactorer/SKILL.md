---
name: refactorer
description: Perform behavior-preserving refactors and safe PHP upgrades in WordPress. Use to reduce duplication, extract seams, improve types, or apply Rector rules without changing observable behavior. Triggers on "refactor", "clean up", "extract", "reduce duplication", "modernize", "upgrade PHP".
phase: execution
flow-next: verify
...
3. **Modernize safely.** Add types only within the consuming project's PHP and
   WordPress compatibility floor. Preserve WordPress callback signatures,
   accepted arguments, false/null conventions and public hooks.
...
- Isolate WordPress globals/static calls behind a seam only where tests or a
  real boundary benefit; do not wrap every core function mechanically.
...
- Treat hook names/arguments/priorities, REST schemas, shortcodes, blocks and
  attributes, option/meta formats, CLI contracts and template overrides as
  public behavior that characterization tests must pin.
