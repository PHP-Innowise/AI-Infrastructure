...
description: Map an existing WordPress codebase into `codebase/` documents that an agent reads instead of the source. Use before planning work in unfamiliar code, when onboarding onto a brownfield project, or when the existing map has drifted from the code. Triggers on "map the codebase", "how is this project structured", "where does X live", "orient me in this project".
phase: understanding
flow-next: architect
...
2. Record `git rev-parse HEAD`. Every document written in this run carries that
   exact commit.
3. Resolve the mapped scope. Default to the repository root because WordPress
   entry points commonly live in a plugin bootstrap, `functions.php`,
   `theme.json`, `block.json`, `includes/`, `src/`, or `app/`; use a narrower
   scope only when the caller names one.
...
| File | Covers |
| --- | --- |
| `STACK.md` | PHP version, WordPress version, key dependencies, tooling |
| `INTEGRATIONS.md` | WordPress/WooCommerce dependencies, external APIs, cron/queues, mail, storage and auth |
| `ARCHITECTURE.md` | Plugin/theme shape, lifecycle hooks, blocks, REST/CLI/admin/frontend entry points and data flow |
| `STRUCTURE.md` | Bootstrap, includes/src/app, templates, blocks/assets, tests/build and where new code belongs |
| `CONVENTIONS.md` | Prefixes/namespaces, hooks, capabilities, escaping, data access and compatibility rules |
| `TESTING.md` | PHP/JS/e2e layout, WordPress bootstrap, factories, environments and commands |
