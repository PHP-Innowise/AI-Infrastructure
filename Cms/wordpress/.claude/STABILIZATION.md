...
- A WordPress convention (hook lifecycle, capability/nonce separation,
  contextual escaping, `$wpdb->prepare()`, compatibility) is violated repeatedly.
...
**Rule:** MUST use `$wpdb->prepare()` for SQL values, derive prefixes from
`$wpdb`, and allowlist dynamic identifiers; never concatenate untrusted input.
**Example:**
