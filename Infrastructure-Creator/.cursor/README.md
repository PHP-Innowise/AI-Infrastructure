...
This is the Cursor edition of the generator. It mirrors the Claude edition's 22 skills exactly and adds Cursor-native wiring.

- **Skills:** `.cursor/skills/<name>/SKILL.md` (byte-identical to the Claude edition, including nested references, assets, and scripts).
- **Agents:** `.cursor/agents/<name>-agent.md` - 22 wrappers with reduced frontmatter (`name` + `description` only).
- **Commands:** `.cursor/commands/{infra-scan,infra-generate,infra-build,infra-adapt}.md` - `name` + `description` frontmatter.
