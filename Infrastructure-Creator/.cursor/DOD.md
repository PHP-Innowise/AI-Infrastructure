...
- [ ] Section 11.1 describes every proposed skill and includes all eight groups (architecture / design / frontend / 15 process including memory-bank / universal / specialty / integrations / domain). Domain skills exist only for cohesive confirmed section 8.11 candidates. Section 11.2 arithmetic matches selected editions.
...
- [ ] `skill-forge` generated all 3 design & interaction skills and all 15 process & workflow skills (including operational `memory-bank`) for every target; frontend and specialty conditions match the profile; domain skills exist only for cohesive confirmed section 8.11 candidates.
...
- [ ] The seeded `memory-bank/` passes its validator and matches section 12 (same count, concepts, and sources); the operational `memory-bank` skill was also generated and wrapped; any drift was reported.
...
- [ ] Exactly 22 generator skills exist in `.claude/skills`, `.cursor/skills`, and `.agents/skills`, including `domain-behavior-scanner`.
- [ ] Corresponding skill files and nested references/assets/scripts are byte-identical across all three editions.
- [ ] Cursor has 22 matching reduced-frontmatter agents; Codex has no agents or commands.
...
- [ ] All 22 skills exist in `.claude/skills`, `.cursor/skills`, and `.agents/skills`, byte-identical where required.
...
- [ ] All 22 skills were re-authored (not left as PHP copies), including `domain-behavior-scanner` and `stack-adapter`'s own identity-swapped copy, and mirrored byte-identically across its three editions.
