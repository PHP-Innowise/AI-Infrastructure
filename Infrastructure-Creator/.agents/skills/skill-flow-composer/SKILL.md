...
Consumes the validated **skill-forge log** and schema **1.6**
...
2. **Read the validated contracts** to obtain the exact generated set,
   ...
   schema 1.0/1.1/1.2/1.3/1.4/1.5 plan; flow compilation requires schema 1.6.
...
```json flow-contract
[exact schema 1.6 flow_contracts object]
```
...
- MUST preserve every adjacency and every schema 1.6 routing oracle; circular
