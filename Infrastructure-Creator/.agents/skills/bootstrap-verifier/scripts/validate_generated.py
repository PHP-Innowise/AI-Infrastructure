...
Checks:
  ...
  - The upgrade contract: .infra-manifest.json exists, parses, carries a
    ...
    matching sha256, and AGENTS.md's first line is the version stamp matching
    the manifest (skipped when a merge-mode target's AGENTS.md pre-existed
    and is untracked). Files the manifest does not list are the team's own
    (infra-update's "untouchable" class - pre-existing in merge mode, or
    added by the team after any generation) and are never a finding: there
    is deliberately no disk-to-manifest coverage walk in any mode.
  - No template placeholders remain in any manifest-tracked
    (generator-owned) file. The team's own files are never scanned.
...
def load_tracked_files(target: Path) -> set | None:
    """The manifest's files map as a set of relative paths, or None.

    This set is the single definition of "generator-owned" for scans that
    must never touch the team's own files (the placeholder scan). None means
    the manifest is missing or unreadable: callers fall back to scanning
    every candidate, and validate_manifest reports the manifest problem
    itself.
    """
    try:
...
def check_placeholders(
    path: Path, errors: list, target: Path | None = None, tracked: set | None = None
) -> None:
    """Report leftover template placeholders in a generator-owned file.

    Ownership comes from the manifest: when a tracked set is available, a
    file absent from it belongs to the target team (pre-existing in merge
    mode, or team-added later) and is never scanned - the word TODO in a
    team AGENTS.md is not a defect of the generation. Without a readable
    manifest (tracked is None) every candidate is scanned, and
    validate_manifest reports the broken manifest itself.
    """
    if tracked is not None and target is not None:
        if path.relative_to(target).as_posix() not in tracked:
            return
    try:
        text = path.read_text(encoding="utf-8")
...
def validate_edition(
    target: Path, edition: str, errors: list, tracked: set | None = None
) -> None:
    ...
    for name in skill_names:
        ...
        check_placeholders(sp, errors, target, tracked)
    ...
    if agents_rel:
        ...
        if agents_dir.exists():
            for af in agents_dir.glob("*.md"):
                ...
                check_placeholders(af, errors, target, tracked)
            for required_skill in MEMORY_QUARTET:
    ...
    if commands_rel:
        ...
        if commands_dir.exists():
            for cf in commands_dir.glob("*.md"):
                ...
                check_placeholders(cf, errors, target, tracked)
            for required_skill in MEMORY_QUARTET:
                if required_skill in skill_names and not (
...
def validate_manifest(target: Path, errors: list) -> None:
    ...
    # Generation mode: "full" (overwrite/fresh) or "merge" (only files the
    # generator actually created are tracked; the team's pre-existing files
    # stay untracked). In both modes the manifest lists generator-owned files
    # only - anything else on disk is the team's property, so a file missing
    # from the manifest is never a finding here (see the coverage note below).
    ...
    # Deliberately NO disk-to-manifest coverage walk, in any mode: a file the
    # manifest does not list is the team's own (infra-update's "untouchable"
    # row - never read, never diffed, never reported), so the team may add
    # its own files under the edition roots at any time without failing a
    # later verification. The loop above already guarantees the real
    # contract: every tracked file exists and matches its sha256.
...
def main() -> int:
    ...
    errors: list = []
    tracked = load_tracked_files(target)
    ...
    for edition in editions:
        validate_edition(target, edition, errors, tracked)
    ...
    check_placeholders(target / "AGENTS.md", errors, target, tracked)
