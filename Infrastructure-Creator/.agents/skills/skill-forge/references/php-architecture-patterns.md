...
- Pattern implications are review concerns, not confirmed target rules. Elevate only claims backed by bounded target anchors; leave unsupported resilience, retry, ordering, ownership, or layering claims as questions.
- Consume scanned path authority, test topology, and exact command definitions. Map every intersecting high-priority invariant ID to a procedure step and verification assertion.
- Include every material adjacent owner and positive, negative, ambiguous, and cross-domain routing case; do not compress architecture routing to one sibling.
...
- **Required evidence:** bounded target-relative anchors for deployable entry points, namespaces/modules/services, representative cross-boundary calls, dependency/wiring configuration, path authority, applicable test topology/commands, and any architecture ADR/tests. Record confidence and contradictions. For an inferred pattern, the necessity rationale must explain why a dedicated skill is useful despite uncertainty.
...
- **Verification:** use the scanned target commands to inspect imports/dependencies/container wiring and run affected contract/architecture tests; assert mapped high-priority invariants where applicable. When no automated architecture check exists, report a concrete manual dependency-edge review rather than inventing a command.
