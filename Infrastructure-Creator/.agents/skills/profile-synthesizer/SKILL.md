...
1. **Load all inputs** from `tasks/TASK-{N}/`: the seven `*-findings.md` (including `domain-behavior-scanner-findings.md`), `stack-researcher-findings.md`, and `clarifying-interview-answers.md`.
...
7. **Derive section 11.1 ("Skills To Generate")** in eight groups, cross-referencing `skill-forge/references/` for what each group actually contains:
   ...
   - **Process & Workflow** (15, always, fixed list): the skills named in `skill-forge/references/php-process-skills.md`, including the operational `memory-bank` skill - list all 15 by name; one shared sentence suffices since this group's mechanic never varies by target.
