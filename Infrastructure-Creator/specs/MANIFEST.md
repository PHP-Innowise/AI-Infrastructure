...
- **Upgradeable output, single version source.** The root `VERSION` file is the only place the generator's version lives; the Project Profile's metadata, the target `AGENTS.md` stamp, and `.infra-manifest.json` all read it. Every generation writes `.infra-manifest.json` into the target (version, source profile, sha256 per generator-owned file; runtime state such as memory chunks, indexes, and counters is deliberately untracked). `infra-update` consumes it with a strict ownership rule: hash-unchanged files are the generator's to replace, hash-changed files are the team's and require an explicit per-file decision, unlisted files are untouchable, and a missing manifest (legacy target, pre-1.4.0) aborts the run. `bootstrap-verifier` validates the whole contract - coverage, hashes, stamp - after every generate/update run.
