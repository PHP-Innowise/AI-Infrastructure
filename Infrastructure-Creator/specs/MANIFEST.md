...
- **Content review-and-repair before publication.** The mechanical gates prove form; `infra-validate` proves substance. After the wrappers and flows exist and before manifest stamping, parallel read-only `content-reviewer` lanes read every staged file - skills, wrappers, policy, hooks, memory seeds - plus the run's own profile and plan, judging each on uniqueness (identity-erasure: a file whose scaffolding would read identically against another PHP repository was copied, not generated), completeness (executable end to end without its author), accuracy (claims match the current target), and coherence (siblings agree). Blocking findings are repaired through the owning forge as contract amendments within at most two rounds - a repair never invents evidence; what evidence cannot settle escalates to re-scan/re-interview - and the mechanical validators re-run after any repair. The review record (`infra-validate-review.json`) is held by the deterministic `validate_content_review.py` gate: full surface coverage, all dimensions answered, verbatim exemptions limited to the sanctioned runtime, no open blockers or escalations. `bootstrap-verifier` requires that record; publication is impossible without it. Standalone `infra-validate` reviews a published target through its manifest and publishes repairs transactionally with a manifest hash refresh.
...
## Skill Inventory (25 in every edition)

- Orchestration: `infra-scan`, `infra-generate`, `infra-validate`, `infra-build`, `infra-update`, `stack-adapter`.
...
- Verification: `content-reviewer`, `bootstrap-verifier`.
