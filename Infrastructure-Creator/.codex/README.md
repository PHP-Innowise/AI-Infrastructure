...
| --- | --- | --- |
| Skills (23 workflows) | `.agents/skills/<name>/SKILL.md` | Codex loads repo skills from `.agents/skills`, not `.codex/`. The complete tree is byte-identical to the Claude edition. |
...
- **No agent wrappers.** Codex invokes the 23 skills directly, so Claude/Cursor wrappers are not mirrored here.
...
Usage: invoke `infra-scan <target-php-project>`, review the profile, then `infra-generate <target>` (or `infra-build <target>`); later, `infra-update <target>` upgrades a generated target via its `.infra-manifest.json`. Every invocation requires an explicit target path. For a confirmed non-PHP target, invoke `stack-adapter <target>` only after explicit user approval.
