...
| --- | --- | --- |
| Skills (20 workflows) | `.agents/skills/<name>/SKILL.md` | Codex loads repo skills from `.agents/skills`, not `.codex/`. Byte-identical to the Claude edition. |
...
- **No command layer.** Codex uses skills directly rather than slash commands; run a skill by name (`infra-scan`, `infra-generate`, `infra-build`).
- **No agent wrappers.** Codex invokes skills directly, so the `.claude/agents` / `.cursor/agents` wrappers are not mirrored here.

Usage: invoke `infra-scan <target-php-project>`, review the profile, then `infra-generate <target>` (or `infra-build <target>`). Every invocation requires an explicit target path.
