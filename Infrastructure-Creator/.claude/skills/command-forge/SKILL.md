...
`tasks/TASK-{N}/skill-generation-plan.json` schema **1.6** (scope,
...
6. **Compile all routing cases.** From the validated contracts, classify
   ...
   every adjacency for each agent. Execute every schema 1.6
...
- MUST compile SKILL FLOW and executable flows from the same schema 1.6
