...
One agent wraps each of the 22 skills. An agent runs exactly one skill in an isolated context and then stops - except the four sanctioned orchestrators (`infra-scan`, `infra-generate`, `infra-build`, `stack-adapter`), which may fan out other skills (see the root `AGENTS.md` Orchestration Exception).
...
The Cursor edition mirrors all 22 agents with reduced frontmatter (`name` + `description` only). Codex has no agent layer and invokes the same 22 skills directly.
