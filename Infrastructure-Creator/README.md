...
- The upgrade contract: a version-stamp comment on the first line of the generated `AGENTS.md` and a `.infra-manifest.json` at the target root (generator version, source profile, sha256 of every generated file) - this is what makes `infra-update` possible later. Commit it with the rest of the accelerator.
...
1. Every `infra-generate` run ends by writing `.infra-manifest.json` into the target - the generator version (from this repo's root `VERSION` file, the single version source), the profile it consumed, and the sha256 of every generated file - and stamping the first line of the generated `AGENTS.md` (`<!-- Generated by Infrastructure-Creator v1.4.0 | TASK-003 | 2026-08-02 -->`).
...
`bootstrap-verifier` runs automatically at the end of `infra-generate` (and `infra-update`) and checks: frontmatter validity across all generated skills/agents/commands, cross-references pointing only at skills that actually exist, hook scripts pass `bash -n` and carry the executable bit with the full per-edition hook set present, every hook wiring file referencing only existing executable scripts, the seeded memory bank passing its own validator, the context-brain runtime smoke-running (`context.py status`/`validate` exit 0 in the generated tree), the `.infra-manifest.json` upgrade contract (present, well-formed, every tracked file existing with a matching hash, `AGENTS.md` stamp agreeing with it), and no template placeholders remaining in any manifest-tracked file. Files the manifest does not list are the target team's own and are never scanned or reported - a team-added file under an edition root can never fail verification. Treat a failed `bootstrap-verifier` run as generation not being done yet.
