...
5. Regardless of the Working result, run
   ...
   root. Indexing refreshes only the disposable SQLite document index.
6. If indexing succeeds, set `procedural`, `semantic`, and `episodic` to
   `updated` and report returned layer/document counts. If it fails, set those
   layers to `failed`; do not roll back a successful lightweight checkpoint.
