...
Create focused, discoverable skills under `.agents/skills/<skill-name>/` without relying on another AI product's CLI or custom-command system.

## Workflow

1. Clarify the capability, trigger phrases, non-goals, expected output, dependencies, and success criteria.
2. Inspect neighboring skills, root `AGENTS.md`, edition documentation, and applicable naming/hooks before editing.
3. Choose a short lowercase kebab-case name that describes the capability.
4. Write `.agents/skills/<name>/SKILL.md` with valid YAML frontmatter and concise imperative instructions.
...
6. Validate syntax, local links, referenced commands, safety constraints, and trigger quality.
7. Test with realistic positive and negative prompts in a fresh Codex context when available. Use automated evaluation only when the user requires measured discovery behavior or benchmarking.

## Improving Existing Skills

1. Inventory the current trigger contract, workflow, resources, native metadata, outputs, safety rules, links, and known evaluation behavior.
...
3. Keep Codex-native frontmatter, repository discovery paths, and skill-name handoffs intact; do not overwrite them with another edition's adapters.
4. Remove a resource only after proving no skill, script, hook, or documentation references it.
...
- Add project workflow metadata only when the repository conventions consume it.
...
- Do not create `.codex/skills`, `.codex/commands`, or Markdown agent wrappers; Codex repository skills live in `.agents/skills`.
...
- authenticated `codex` CLI available on `PATH`, or a compatible executable configured through `SKILL_CREATOR_CODEX_CLI`;
...
```json
[
  {"query": "Create a repository skill for API contract reviews", "should_trigger": true},
  {"query": "Fix this Doctrine query", "should_trigger": false}
]
```

From `.agents/skills/skill-creator/`, run:
...
The harness creates a unique probe skill in a temporary minimal workspace, invokes `codex exec` in an ephemeral read-only sandbox, and counts an exact marker emitted only when the probe body loads. The temporary workspace is removed automatically, and the harness fails closed on CLI errors. Use `--model` only to override the CLI's configured model.
...
Report the skill path, trigger contract, resources added, validation performed, evaluation prompts/results, and any remaining tool-specific limitations. Distinguish syntax tests, deterministic adapter tests, live native evaluations, and manual review.
