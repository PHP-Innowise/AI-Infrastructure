...
find /tmp/claude-loop-detection -type f -delete 2>/dev/null || true
...
echo ""
echo "Structure:"
for DIR in src app public config bin templates views tests specs tasks memory-bank project-brain examples .claude; do
  if [ -d "$DIR" ]; then
